...
Вот обновленный код с кнопкой "Preview":

**Файл: `src/main/kotlin/com/example/openfilesviewer/OpenFilesPanel.kt`**
...
1. **Добавлена кнопка "Preview"** в панель с кнопками
2. **Функция `createPreviewFile()`** - создает markdown файл с содержимым всех открытых файлов
3. **Функция `generateMarkdownContent()`** - генерирует markdown контент:
    - Заголовок с датой создания
    - Для каждого файла: название, путь, содержимое в code block с подсветкой синтаксиса
4. **Функция `getLanguageFromExtension()`** - определяет язык для подсветки синтаксиса по расширению файла
5. **Обработка ошибок** - показывает сообщения об ошибках через диалоговые окна

Файл создается как временный файл и автоматически открывается в редакторе. Markdown файл будет содержать:
- Заголовок и дату создания
- Для каждого открытого файла:
    - Название файла как заголовок
    - Полный путь к файлу
    - Содержимое файла в code block с соответствующей подсветкой синтаксиса
